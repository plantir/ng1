    ...
    angular
        ...
        .controller('gridController', ['$scope', '$element', '$filter', '$location', 'filtersFactory', function ($scope, $element, $filter, $location, filtersFactory) {
            ...
            $scope.defaultsPaginationOptions = {
                perPage: $scope.paginationOptions.perPage || '10',
                page: $scope.paginationOptions.page || 1
            };
            ...
            $scope.changePerPage = function () {
                if ($scope.oldPerPage != $scope.paginationOptions.perPage) {
                    $scope.oldPerPage = $scope.paginationOptions.perPage
                    $scope.paginationOptions.page = 1;
                    $scope.reloadGrid();
                }
            }
            ...
            $scope._gridActions.doCheck = $scope.doCheck;
            ...
            function changePath(isDefaultSort) {
                ...
                path = 'page=' + $scope.paginationOptions.page;
                if ($scope.paginationOptions.perPage !== $scope.defaultsPaginationOptions.perPage) {
                    path += '&perPage=' + $scope.paginationOptions.perPage;
                }

                if ($scope.sortOptions.predicate) {
                    path += '&sort=' + encodeURIComponent(($scope.sortOptions.direction === 'asc' ? '' : '-') + $scope.sortOptions.predicate);
                }
                ...
                $scope.filters.forEach(function (filter) {
                    ...
                    if (value) {
                        ...
                        let filter = `${urlName}:${value}:like`
                        $scope.filterArray.push(filter)
                    }
                });
                if ($scope.filterArray.length) {

                    path += '&filters=' + encodeURIComponent(JSON.stringify($scope.filterArray));
                }
                ...
                for (const key in currentLocation) {
                    if (key !== "page" && key !== "perPage" && key !== "sort" && key !== "filters") {
                        path += `&${key}=${currentLocation[key]}`
                    }
                }
            ...
            function parseUrl() {
                ...
                //     if (key !== 'page' && key !== 'sort' && key !== 'perPage') {
                ...
                if (params.filters) {
                    ...
                    $scope.filters.forEach(function (filter) {
                        var urlName = filter.model,
                            ...
                            regResult = regex.exec(params.filters),
                            value = regResult && regResult[1]
                ...
                //pagination options
                $scope.paginationOptions.perPage = $scope.defaultsPaginationOptions.perPage;
                $scope.oldPerPage = $scope.defaultsPaginationOptions.perPage;
                $scope.paginationOptions.page = $scope.defaultsPaginationOptions.page;

                if (params.perPage) {
                    $scope.paginationOptions.perPage = params.perPage;
                    $scope.oldPerPage = params.perPage;
                }

                if (params.page) {
                    if (!$scope.serverPagination && ((params.page - 1) * $scope.paginationOptions.perPage > $scope.filtered.length)) {
                        $scope.paginationOptions.page = 1;
                    } else {
                        $scope.paginationOptions.page = params.page;
                    }
                }
                ...
                if (params.sort) {
                    ...
                    if (sort.length == 2) {
                        $scope.sortOptions.predicate = decodeURIComponent(sort[1]);
            ...
            function getData() {
                $scope._gridOptions.loading = true;
                var url = '';
                var params = $location.search();
                if ($scope.customFilters && $scope.customFilters.length) {
                    params.filters = params.filters ? JSON.parse(params.filters) : []
                    params.filters = params.filters.concat($scope.customFilters)
                ...
                if (!url && $scope.sortOptions.predicate) {
                    ...
                    $scope._gridOptions.getData(params, function (res) {
                        $scope.filtered = res.data;
                        $scope.paginationOptions.total = res.total;
                        $scope._gridOptions.loading = false;
                    });
                }
            ...
            function applyFilters() {
                ...
                $scope.paginationOptions.total = $scope.filtered.length;
            }
            ...
            return {
                ...
                link: function ($scope, element, attrs, ctrl, transclude) {
                    if ($scope.serverPagination) {
                        ...
                        element.attr('ng-repeat', "item in filtered | startFrom:(paginationOptions.page-1)*paginationOptions.perPage | limitTo:paginationOptions.perPage track by $index");
                    }
        ...
        .factory('filtersFactory', function () {
            ...
            function textFilter(items, value, predicate) {
                var predicateArray = predicate.split('.')
                if (predicateArray.length == 1) {
                    ...
                    return items.filter(function (item) {
                        return value && item[predicateArray[0]][predicateArray[1]] ? ~(item[predicateArray[0]][predicateArray[1]] + '').toLowerCase().indexOf((value + '').toLowerCase()) : !!item[predicateArray[0]][predicateArray[1]];
                    });
                }
            }
